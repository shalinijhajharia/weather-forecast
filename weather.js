...
var api='6f5b3285f541fbfc8c79c870651a8d05';
var city=document.querySelector(".search-box");
var msg=document.getElementById("msg");
city.addEventListener("keydown",(weather)=>{
    if(weather.key==="Enter")
    {
        let url=`https://api.openweathermap.org/data/2.5/weather?q=${city.value}&appid=${api}&units=metric`
        fetch(url)
            .then(response=>response.json())
            .then(data=>display(data))
            .catch(()=>
            {
                msg.textContent="Enter the city name";
            })
    }
})
var display = data => {
